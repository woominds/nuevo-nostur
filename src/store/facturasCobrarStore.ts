// src/store/facturasCobrarStore.ts
...
type FacturasCobrarState = {
  ...
  adjuntarArchivoFactura: (
    factura: FacturaCobrar,
    archivo: File
  ) => Promise<boolean>;
  ...
  setFilter: <
    K extends keyof FacturasCobrarFilters
  >(
    key: K,
  ...
  getCarritosDisponiblesForDraft: (
    draft: DocumentoDraft
  ) => CarritoDisponible[];
...
function calculateDocumentTotalsFromDraft(
  draft: DocumentoDraft
) {
  const sign =
...
function buildVirtualControlFromCarrito(
  carrito: CarritoDisponible
): ControlCarritoDisponible {
  const utilidadNeta =
    getNumber(
  ...
  const importeFacturar =
    getNumber(
      carrito.importe_facturar
    );

  return {
    id:
      `virtual-control-${carrito.id}`,
    carrito_id:
      carrito.id,
    utilidad_almundo:
      utilidadNeta,
...
async function uploadDocumentoArchivo(
  ...
  const path =
    `${currentUserId}/${Date.now()}-${numero}-${fileName}`;
...
export const useFacturasCobrarStore = create<FacturasCobrarState>(
  (set, get) => ({
    loading: false,
    saving: false,
    error: null,
    ...
    loadFacturas: async () => {
      ...
      if (
        ...
        set({
          ...
          selectedFacturaIds: [],
          error:
            "Tu usuario no tiene acceso al módulo Facturas a Cobrar."
        });
      ...
      let facturasQuery =
        supabase
          ...
          .select(
            "*, facturas_cobrar_carritos(*), facturas_cobrar_retenciones(*)"
          )
          .eq(
            "anio",
            filters.anio
      ...
      if (
        ...
        if (
          ...
          facturasQuery =
            facturasQuery.or(
              "estado.eq.COBRADA,cobrado.eq.true"
            );
        }
      ...
      let retencionesQuery =
        supabase
          .from(
            "v_facturas_cobrar_retenciones"
          )
          .select("*")
          .gte(
            "fecha_cobro",
      ...
      let carritosQuery =
        supabase
          .from("carritos")
          .select(
            "*, clientes:cliente_id(*), facturas_cobrar_carritos(*)"
          )
          .order(
            "fecha_venta",
            {
      ...
      const carritosDisponibles =
        (
          (
            carritosRes.data ||
            []
          ) as CarritoDisponible[]
        )
          .filter(
            (carrito) => {
              const estado =
              ...
              const yaTieneFacturaVinculada =
                Array.isArray(
                  carrito.facturas_cobrar_carritos
                ) &&
                carrito
                  .facturas_cobrar_carritos
                  .length > 0;
          ...
          .map(
            (carrito) => ({
              ...carrito,
              carritos_control: [
                buildVirtualControlFromCarrito(
                  carrito
                )
              ]
            })
    ...
    createDocumento: async (
      ...
      if (
        !get().canManageFacturas
      ) {
        set({
          saving: false,
          error:
            "No tenés permisos para crear documentos."
        });
      ...
      const isFacturaCarritos =
        draft.tipo_documento ===
        "FACTURA";

      let selectedCarritos:
        CarritoDisponible[] = [];
      ...
      if (
        ...
        selectedCarritos =
          get()
            ...
            .filter(
              (carrito) =>
                draft.selectedCarritoIds.includes(
                  carrito.id
                )
            );

        if (
          selectedCarritos.length ===
          0
        ) {
          set({
            saving: false,
            error:
              "Seleccioná al menos un carrito para facturar."
          });
      ...
      const suggestedTotalCarritos =
        selectedCarritos.reduce(
          (
            total,
            carrito
          ) => {
      ...
      const draftForTotals:
        ...
        ...draft,
        neto_gravado:
          draft.neto_gravado.trim() ||
          (
            isFacturaCarritos
              ? String(
                  suggestedNetoCarritos
                )
              : ""
          )
      };
      ...
      const totals =
        calculateDocumentTotalsFromDraft(
          draftForTotals
        );
      ...
      const payload = {
        ...
        archivo_url:
          archivoPayload?.archivo_url ||
          null,

        archivo_path:
          archivoPayload?.archivo_path ||
          null,

        archivo_nombre:
          archivoPayload?.archivo_nombre ||
          null,

        archivo_tipo:
          archivoPayload?.archivo_tipo ||
          null,

        archivo_size:
          archivoPayload?.archivo_size ||
          null,
      ...
      const factura =
        facturaRes.data as FacturaCobrar;

      if (
        isFacturaCarritos
      ) {
        const relaciones =
          selectedCarritos.map(
            (carrito) => {
              const control =
                carrito
                  .carritos_control?.[0];
      ...
      if (
        ...
        set({
          saving: false,
          error:
            "No tenés permisos para adjuntar archivos."
        });
      ...
      async (
        ...
        if (
          ...
          set({
            saving: false,
            error:
              "No tenés permisos para registrar cobros."
          });

          return false;
        }

        const selectedFacturas =
          get().getSelectedFacturas();
        ...
        if (
          ...
          set({
            saving: false,
            error:
              "Una o más facturas seleccionadas ya están cobradas."
          });
        ...
        if (!formaPago) {
          set({
            saving: false,
            error:
              "Seleccioná una forma de pago válida."
          });
        ...
        if (!sameMoneda) {
          set({
            saving: false,
            error:
              "Para registrar un cobro agrupado, todas las facturas deben ser de la misma moneda."
          });
        ...
        if (!sameSucursal) {
          set({
            saving: false,
            error:
              "Para registrar un cobro agrupado, todas las facturas deben ser de la misma sucursal."
          });
        ...
        if (
          ...
          set({
            saving: false,
            error:
              "Esta forma de pago impacta tesorería. Seleccioná la caja o banco destino."
          });
        ...
        if (
          ...
          set({
            saving: false,
            error:
              `La caja seleccionada es ${caja.moneda} y las facturas están en ${moneda}.`
          });
        ...
        if (
          ...
          set({
            saving: false,
            error:
              "El total de las facturas seleccionadas debe ser mayor a cero."
          });

          return false;
        }

        const importeIngresadoBanco =
          impactaTesoreria
            ? parseMoney(
                draft.importe_ingresado_banco ||
                  totalFacturas
              )
            : totalFacturas;
        ...
        if (
          impactaTesoreria &&
          importeIngresadoBanco <=
            0
        ) {
          set({
            saving: false,
            error:
              "Ingresá el importe que entró realmente a la caja o banco."
          });
        ...
        if (
          ...
          set({
            saving: false,
            error:
              "El importe ingresado no puede superar el total de las facturas seleccionadas."
          });
        ...
        let acumuladoCobrado =
          0;

        let acumuladoRetencion =
          0;

        const items =
          selectedFacturas.map(
            (
              factura,
              index
            ) => {
          ...
          Array.from(
            new Set(
              selectedFacturas
                .flatMap(
                  (factura) =>
                    factura.facturas_cobrar_carritos ||
                    []
                )
                .map(
                  (item) =>
        ...
        const {
          ...
          "registrar_cobro_facturas_cobrar",
          {
            p_fecha_cobro:
              fechaCobro,
    ...
    anularCobro: async (
      ...
      if (
        ...
        set({
          saving: false,
          error:
            "No tenés permisos para anular cobros."
        });
      ...
      if (
        ...
        set({
          saving: false,
          error:
            "La factura seleccionada no está cobrada."
        });
    ...
    toggleFacturaSelection: (
      ...
      if (
        ...
        if (
          ...
          set({
            error:
              "Solo podés agrupar facturas de la misma moneda."
          });
        ...
        if (
          ...
          set({
            error:
              "Solo podés agrupar facturas de la misma sucursal."
          });
      ...
      set((state) => {
        const exists =
          state.selectedFacturaIds.includes(
            facturaId
          );
        ...
        return {
          selectedFacturaIds:
            exists
              ? state.selectedFacturaIds.filter(
                  (id) =>
                    id !==
                    facturaId
                )
              : [
                  ...state.selectedFacturaIds,
                  facturaId
                ],
          selectedFacturaId:
            facturaId,
    ...
    getSelectedFacturas: () => {
      const selectedIds =
        get().selectedFacturaIds;

      return get().facturas.filter(
        (factura) =>
    ...
    getCarritosDisponiblesForDraft: (
      ...
      return get().carritosDisponibles.filter(
        (carrito) => {
          const controles =
            Array.isArray(
              carrito.carritos_control
            )
              ? carrito.carritos_control
              : [];
          ...
          const carritoFacturado =
            Boolean(
              carrito.facturado
            ) ||
            yaTieneFacturaVinculada;

          const carritoCobrado =
            Boolean(
              carrito.cobrado
            );

          const carritoCancelado =
            Boolean(
              carrito.cancelado
            ) ||
            estado ===
      ...
      facturas.forEach(
        (factura) => {
          ...
          if (
            ...
            monedaGrupo.totalSeleccionado +=
              importe;

            monedaGrupo.cantidadSeleccionada +=
              1;
          }
        }
      );
